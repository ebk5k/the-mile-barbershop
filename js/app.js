...
(function () {
  ...
  function renderPhotos() {
    ...
    if (feedEl) feedEl.innerHTML = feed.map((p, i) => `<button class="card-photo" type="button" role="listitem" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/feed/${p.id}.webp" width="1080" height="1350" loading="${i < 2 ? 'eager' : 'lazy'}" decoding="async" alt="${cap(p)}"><span class="expand" aria-hidden="true">${EXPAND}</span><span class="cap"><b>${cap(p)}</b><i>${i + 1}/${feed.length}</i></span></button>`).join('');
    const nails = M.photos.filter((p) => p.kind === 'nail');
    if (nailEl) nailEl.innerHTML = nails.map((p) => `<button class="tile" type="button" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/feed/${p.id}.webp" width="1080" height="1350" loading="lazy" decoding="async" alt="${cap(p)}"></button>`).join('');
    if (gridEl) gridEl.innerHTML = w.map((p) => `<button class="tile" type="button" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/sq/${p.id}.webp" width="900" height="900" loading="lazy" decoding="async" alt="${cap(p)}"></button>`).join('');
