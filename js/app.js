...
(function () {
  const M = window.MILE; const AV = '?v=' + (M.assetVersion || '1'); const $ = (s, r) => (r || document).querySelector(s); const $$ = (s, r) => [...(r || document).querySelectorAll(s)];
  ...
  function renderPhotos() {
    ...
    if (feedEl) feedEl.innerHTML = feed.map((p, i) => `<button class="card-photo" type="button" role="listitem" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/feed/${p.id}.webp${AV}" width="864" height="1152" loading="${i < 2 ? 'eager' : 'lazy'}" decoding="async" alt="${cap(p)}"><span class="expand" aria-hidden="true">${EXPAND}</span><span class="cap"><b>${cap(p)}</b><i>${i + 1}/${feed.length}</i></span></button>`).join('');
    const nails = M.photos.filter((p) => p.kind === 'nail');
    if (nailEl) nailEl.innerHTML = nails.map((p) => `<button class="tile" type="button" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/feed/${p.id}.webp${AV}" width="864" height="1152" loading="lazy" decoding="async" alt="${cap(p)}"></button>`).join('');
    if (gridEl) gridEl.innerHTML = w.map((p) => `<button class="tile" type="button" data-open-viewer="${M.photos.indexOf(p)}" aria-label="${cap(p)}"><img src="assets/img/sq/${p.id}.webp${AV}" width="864" height="864" loading="lazy" decoding="async" alt="${cap(p)}"></button>`).join('');
    // viewer slides (built once; captions refreshed per language)
    const vs = $('#viewer-scroll');
    if (vs && !vs.children.length) vs.innerHTML = allViewable().map((p, i) => `<div class="slide" data-i="${i}"><img src="assets/img/feed/${p.id}.webp${AV}" loading="lazy" decoding="async" alt=""><div class="cap"><div><b data-cap="${i}"></b><i>${M.name} · Huntington Park</i></div><a class="btn primary small" href="tel:${M.phone.tel}"><span data-callcap>Call</span></a></div></div>`).join('');
